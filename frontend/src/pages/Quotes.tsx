...
export const Quotes = () => {
  ...
  const { data: quotes = [], isLoading, isError, error, refetch } = useQuery({
    queryKey: queryKeys.quotes.list(50),
    queryFn: () => quotesApi.list(50).then(res => res.data),
          ...
          <FadeIn>
            <div className="bg-white border border-gray-200 rounded-lg p-12 text-center">
              <p className="text-red-600 mb-4">
                {error instanceof Error ? error.message : 'Unable to load quotes. Check that the backend is running and try again.'}
              </p>
              <button
                onClick={() => refetch()}
